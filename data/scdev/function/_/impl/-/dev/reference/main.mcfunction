...
data modify storage scdev:_ v.reference.lines append value {text:"        └ ", color:gray, extra:[{text:"uninstall.json", color:aqua, bold:true, hover_event:{action:"show_text", value:{text:"", color:gray, extra:[{text:"#<pack id>:uninstall", italic:true}, {text:"\n\n"}, {text:"Called when the datapack is uninstalled by SlimeCore; assumedly before the user removes the datapack from the world.\nIdeally, a pack should erase all trace when this tag is called, including (but not limited to) storage NBT, scoreboards, and entities."}]}}}]}
