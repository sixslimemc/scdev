...
data modify storage scdev:_ v.reference.lines append value {text:"        │ └ ", color:gray, extra:[{text:"", extra:[{text:"<id>", color:gold, italic:true, bold:true}, {text:".json", color:gold, italic:false, bold:true}], hover_event:{action:"show_text", value:{text:"", color:gray, extra:[{text:"#<pack id>:entrypoint/<id>", italic:true, color:gold}, {text:"\n"}, {text:"Called after (all) load tags are called.\n- One tag per entrypoint declared by the manifest, with tag names matching entrypoint ids.\n- Called in the same order specified by the manifest.\n- Should represent and start non-initialization work (e.g. schedule loops)."}]}}}, {text:" (...)", color:dark_gray, bold:false, hover_event:{action:"show_text", value:{text:"", color:dark_gray, extra:[{text:"One tag per entrypoint declared by the manifest, with tag names matching entrypoint ids."}]}}}]}
