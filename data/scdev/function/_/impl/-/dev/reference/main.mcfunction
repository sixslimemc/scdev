...
data modify storage scdev:_ v.reference.lines append value {text:"        │ └ ", color:gray, extra:[{text:"", extra:[{text:"<id>", color:gold, italic:true, bold:true}, {text:".json", color:gold, italic:false, bold:true}], hover_event:{action:"show_text", value:{text:"", color:gray, extra:[{text:"#<pack id>:entrypoint/<id>", italic:true, color:gold}, {text:"\n"}, {text:"- One tag per entrypoint declared in your pack's manifest (tag name must match the 'id').\n- Called after all packs 'load' tags are called.\n- Standard tick/schedule loops should be started in entrypoints."}]}}}, {text:" (...)", color:dark_gray, bold:false, hover_event:{action:"show_text", value:{text:"", color:dark_gray, extra:[{text:"One tag per entrypoint declared in your pack's manifest (tag name must match the 'id')."}]}}}]}
