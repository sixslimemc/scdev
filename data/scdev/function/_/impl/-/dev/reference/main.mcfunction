...
data modify storage scdev:_ v.reference.lines append value {text:"        │ └ ", color:gray, extra:[{text:"", extra:[{text:"<id>", color:dark_purple, italic:true, bold:true}, {text:".json", color:dark_purple, italic:false, bold:true}], hover_event:{action:"show_text", value:{text:"", color:gray, extra:[{text:"#<pack id>:preload_entrypoint/<id>", italic:true, color:dark_purple}, {text:"\n"}, {text:"- Like entrypoints, but called just *before* load tags are called (including your pack's load tag).\n- Avoid use without very good reason (e.g. very technical or meta work)."}]}}}, {text:" (...)", color:dark_gray, bold:false, hover_event:{action:"show_text", value:{text:"", color:dark_gray, extra:[{text:"One tag per preload entrypoint declared by your manifest, with tag names matching preload entrypoint ids."}]}}}]}

# '#<pack_id>:load':
data modify storage scdev:_ v.reference.lines append value {text:"        ├ ", color:gray, extra:[{text:"load.json", color:aqua, bold:true, hover_event:{action:"show_text", value:{text:"", color:gray, extra:[{text:"#<pack id>:load", italic:true, color:aqua}, {text:"\n"}, {text:"- Called when your datapack is loaded (conceptually similar to #minecraft:load).\n- Your pack should do strict inititialization work in this tag (declare scoreboards, initialize defaults, etc.), but do all other work (start tick/schedule loops, independent processes, etc.) in entrypoints.\n- Dependencies are always loaded before their their dependants."}]}}}]}

# '#<pack_id>:disable':
data modify storage scdev:_ v.reference.lines append value {text:"        ├ ", color:gray, extra:[{text:"disable.json", color:aqua, bold:true, hover_event:{action:"show_text", value:{text:"", color:gray, extra:[{text:"#<pack id>:disable", italic:true, color:aqua}, {text:"\n"}, {text:"- Called just before your pack is disabled.\n- Your pack should gracefully stop itself in this tag, with the assumption that it may be re-enabled later.\n- Will always be called at some point before your pack is uninstalled.\n- Dependencies will never be disabled before their dependants."}]}}}]}
