...
data modify storage scdev:_ v.reference.lines append value {text:"        ├ ", color:gray, extra:[{text:"load.json", color:aqua, bold:true, hover_event:{action:"show_text", value:{text:"", color:gray, extra:[{text:"#<pack id>:load", italic:true, color:aqua}, {text:"\n"}, {text:"Called on pack load--analagous to legacy #minecraft:load. Dependencies are always loaded before their dependants.\nA pack should initialize itself for proper function, but should "}, {text:"NOT", color:red, bold:true}, {text:" start any schedule/tick loops."}]}}}]}
