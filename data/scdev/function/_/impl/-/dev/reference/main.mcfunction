...
data modify storage scdev:_ v.reference.lines append value {text:"        │ └ ", color:gray, extra:[{text:"", extra:[{text:"<id>", color:gold, italic:true, bold:true}, {text:".json", color:gold, italic:false, bold:true}], hover_event:{action:"show_text", value:{text:"", color:gray, extra:[{text:"#<pack id>:entrypoint/<id>", italic:true, color:gold}, {text:"\n"}, {text:"- One tag per entrypoint declared in your pack's manifest, with tag names matching entrypoint ids.\n- Called after ALL packs 'load' tags are called, in the order the entrypoints are specified in your pack's manifest.\n- Non-initialization work and tick/schedule loops should be started by entrypoints."}]}}}, {text:" (...)", color:dark_gray, bold:false, hover_event:{action:"show_text", value:{text:"", color:dark_gray, extra:[{text:"One tag per entrypoint declared in your pack's manifest (tag name must match the 'id')."}]}}}]}
