...
data modify storage scdev:_ v.reference.lines append value {text:"        ├ ", color:gray, extra:[{text:"disable.json", color:aqua, bold:true, hover_event:{action:"show_text", value:{text:"", color:gray, extra:[{text:"#<pack id>:disable", italic:true, color:aqua}, {text:"\n"}, {text:"Called just before this pack is disabled. Dependencies will never be disabled before their dependants.\nA pack should attempt to safely remove and/or store it's content, keeping in mind that it may be re-enabled OR uninstalled in the future."}]}}}]}
