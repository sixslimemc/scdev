...
data modify storage scdev:_ v.reference.lines append value {text:"        │ └ ", color:gray, extra:[{text:"", extra:[{text:"<id>", color:gold, italic:true, bold:true}, {text:".json", color:gold, italic:false, bold:true}], hover_event:{action:"show_text", value:{text:"", color:gray, extra:[{text:"#<pack id>:entrypoint/<id>.json", italic:true, color:gold}, {text:"\n"}, {text:"Called after all packs are loaded. Entrypoint tags are called in the order they are specified in the manifest, respecting all explicitly specified before/after relationships to dependencies' entrypoints.\nAny and all schedule loops and non-initialization work should be started and conceptually represented by entrypoints."}]}}}, {text:" (...)", color:dark_gray, bold:false}]}
