...
data modify storage scdev:_ v.reference.lines append value {text:"        │ └ ", color:gray, extra:[{text:"", extra:[{text:"<id>", color:dark_purple, italic:true, bold:true}, {text:".json", color:dark_purple, italic:false, bold:true}], hover_event:{action:"show_text", value:{text:"", color:gray, extra:[{text:"#<pack id>:preload_entrypoint/<id>", italic:true, color:dark_purple}, {text:"\n"}, {text:"- Like entrypoints, but called *before any* packs' load tags are called, including the parent pack's load tag.\n- Should generally avoid utilizing without very good reason (e.g. very technical or meta work)."}]}}}, {text:" (...)", color:dark_gray, bold:false, hover_event:{action:"show_text", value:{text:"", color:dark_gray, extra:[{text:"One tag per preload entrypoint declared by your manifest, with tag names matching preload entrypoint ids."}]}}}]}
