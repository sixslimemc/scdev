...
# ignore info line:
execute unless data storage scdev:_ v.rebuild.args{ignore_warnings:true} run data modify storage scdev:_ v.rebuild.lines append value {text:"", color:gray, italic:true, extra:[{text:"Rebuild aborted due to above warnings."}], hover_event:{action:'show_text', value:{text:"", color:gray, extra:[{text:"Set "}, {text:"{args:{ignore_warnings:true}}", color:dark_aqua}, {text:" to rebuild regardless of warnings. This will not \"break\" anything; just make sure your inputs are intentional."}]}}}
