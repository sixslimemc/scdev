...
# check/set {..installed}:
data remove storage scdev:_ v.dependency.installed
$data modify storage scdev:_ v.dependency.installed set from storage slimecore:data world.raw_manifests[{pack_id:"$(pack_id)"}]
...
execute store success score *x _scdev run data modify storage scdev:_ x.author set from storage scdev:_ v.dependency.installed.author_id
execute if score *x _scdev matches 1 run return 3

# set {..installed_ver_text}:
data modify storage scdev:in version.value set from storage scdev:_ v.dependency.installed.version
...
data modify storage slimecore:in version_req.version set from storage scdev:_ v.dependency.installed.version
function slimecore:eval/version_req
execute if data storage slimecore:out version_req.result.error run return 4

# check disabled:
$execute store success score *x _scdev if data storage slimecore:data world.aux.installed_map.'$(pack_id)'{disabled:true}
