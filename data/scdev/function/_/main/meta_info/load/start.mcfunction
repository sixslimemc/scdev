...
execute unless data storage scdev:_ t.load.preloads[0] run tellraw @a[tag=scdev.watch] {text:"(none)", italic:true, color:dark_gray}
...
execute unless data storage scdev:_ t.load.entrypoints[0] run tellraw @a[tag=scdev.watch] {text:"(none)", italic:true, color:dark_gray}
