...
tellraw @a[tag=scdev.watch] {text:" Preload Entrypoints:", color:white, bold:false, underlined:true}
...
tellraw @a[tag=scdev.watch] {text:" Packs:", color:white, bold:true, underlined:true}
data modify storage scdev:_ t.load.loads set from storage slimecore:data build.order.load
execute unless data storage scdev:_ t.load.loads[0] run tellraw @a[tag=scdev.watch] {text:"  (none)", color:dark_gray}
execute if data storage scdev:_ t.load.loads[0] run function scdev:_/main/meta_info/load/start/loads/each

tellraw @a[tag=scdev.watch] {text:" Entrypoints:", color:white, bold:false, underlined:true}
