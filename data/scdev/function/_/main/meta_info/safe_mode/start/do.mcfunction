...
data modify storage scdev:_ t.safe_mode.lines append value {text:"\"", color:light_purple, extra:[{text:"Safe-load finished.\" should be sent below."}], hover_event:{action:"show_text", value:[{text:"If no message is sent:\n - 'max_command_sequence_length' gamerule may need to be increased.\n - One or more packs may have an infinite loop in their safe mode function(s).", color:gray}]}}
